...
### 히어로 배경
히어로 원본(1920×900)은 **그림이 오른쪽 60~100% 구간에만** 있고 왼쪽 절반은 단색 남색입니다.
PC에서는 가로로 넓어 그림이 보이지만, 폰에서는 세로로 긴 영역에 `cover`로 맞춰지면서
가운데(빈 남색)만 잘려 보여 허전했습니다.

그래서 **900px 이하에서만** 보여줄 영역을 오른쪽으로 옮깁니다.

```css
background:
  linear-gradient(176deg,
    rgba(13,27,62,0.94) 0%,    /* 글자가 얹히는 위쪽은 진하게 */
    rgba(13,27,62,0.86) 45%,
    rgba(13,27,62,0.74) 72%,
    rgba(13,27,62,0.30) 100%   /* 아래쪽은 옅게 해서 그림이 드러나게 */
  ),
  url("./uploads/hero.jpg") 82% center / cover;
```

**히어로 사진을 교체하면 `82%` 값을 다시 맞춰야 합니다.**
새 사진의 주요 피사체가 가운데 있다면 `center`로 되돌리세요.
그라디언트 농도는 흰 글자가 읽히는 선에서 맞춘 값이므로, 사진을 바꾸면 함께 확인해야 합니다.
